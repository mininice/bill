...
function template(config) {
  if(!config.hasCheckbox || config.hasCheckbox.visible == false) {
    config.hasCheckbox = defaultConfig.hasCheckbox;
  }
  ...
  myDiagram.nodeTemplate =
    $(go.Node, "Table",
      ...
      $(go.Panel, 'Auto', {row: 0, column: 0, padding: 3},
        $(go.Shape, "circle", {
            fill: "#fff",
            stroke: config.hasCheckbox.default, 
            width: config.hasCheckbox.size, 
            height: config.hasCheckbox.size, 
            strokeWidth: config.hasCheckbox.strokeWidth,
            visible: config.hasCheckbox.visible
            //fill: '#fff'
          },
          new go.Binding('stroke', 'checked', function(d) {
            if(d === true) {
              return config.hasCheckbox.checked;
            } else {
              return config.hasCheckbox.default;
            }
          })
        ),
        $(go.TextBlock, {
        ...
        new go.Binding('stroke', 'checked', function(d) {
          return d ? config.hasCheckbox.checked : config.hasCheckbox.default;
        }),
        ...
        $(go.Shape, "LineH",
          {
            ...
            strokeWidth: config.lineWeight, height: config.lineWeight,
          ...
          new go.Binding("width", "text", function(d) {  
            return getLineWidth(d, config.wordLineSpead);
          }),
  ...
  myDiagram.nodeTemplateMap.add("Comment",
    $(go.Node,  // this needs to act as a rectangular shape for BalloonLink,
      ...
      $(go.Panel, "Auto",
      // this Adornment has a rectangular blue Shape around the selected node
        $(go.Shape, { figure: "File", fill: '#F7F6B0', stroke: "#fff", strokeWidth: 1}
        ),
        $(go.TextBlock, {editable: true},
        { stroke: "brown", stroke: "orange", margin: 3},
        new go.Binding("text"))
      )
  ));
